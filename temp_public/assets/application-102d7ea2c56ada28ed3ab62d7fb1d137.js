...
(function() {
  ...
  jQuery(function() {
    return $(document).on("page:change", function() {
      ...
      return $('.navbar input.form-control').val('');
    });
  });
...
(function() {
  ...
  jQuery(function() {
    return root.UserIndex = function() {};
  });
  ...
  jQuery(function() {
    return root.UserNew = function() {
      ...
      $('form').submit(function(event) {
        var day_rgx, flag, month_rgx, phone_rgx, year_rgx;
        phone_rgx = new RegExp("^[0-9]{3,4}$");
        year_rgx = new RegExp("^(1|2)[0-9]{3}$");
        month_rgx = new RegExp("^(0[1-9]|1[0-2])$");
        day_rgx = new RegExp("^(0[1-9]|(1|2)[0-9]|3[0-1])$");
        flag = false;
        ...
        $(this).find("input[data-type='year']").each(function() {
          if (!year_rgx.test($(this).val())) {
            alert($("label[for=user_birthday]").text() + "중 년이 올바르지 않습니다.");
            return flag = true;
          }
        });
        $(this).find("input[data-type='month']").each(function() {
          if (!month_rgx.test($(this).val())) {
            alert($("label[for=user_birthday]").text() + "중 월이 올바르지 않습니다.");
            return flag = true;
          }
        });
        $(this).find("input[data-type='day']").each(function() {
          if (!day_rgx.test($(this).val())) {
            alert($("label[for=user_birthday]").text() + "중 일이 올바르지 않습니다.");
            return flag = true;
          }
        });
        if (flag) {
      ...
      $('#new-search-user').on("click", function() {
        $.ajax('/user/find_user', {
          ...
          error: function(jqXHR, textStatus, errorThrown) {
            $('i[data-toggle=true').toggle();
            return $('#new-result-panel').append("<a class='list-group-item list-group-item-danger'>예기치 않은 에러 발생. 다시 시도 해 주십시오.</a>");
          },
          success: function(data, textStatus, jqXHR) {
            ...
            data.forEach(function(elem, idx, arr) {
              return $('#new-result-panel').append("<a class='list-group-item' data-id='" + elem.id + "'> 이름: <strong>" + elem.name + "</strong> &nbsp;&nbsp; 전화번호: <strong>" + elem.phone_number + "</strong> &nbsp;&nbsp; 생년월일: <strong>" + elem.birthday + "</strong></a>");
            });
          }
        });
        return false;
...
(function() {
  ...
  jQuery(function() {
    root.UserShow = function() {
      $('.toggling-panel').on("click", "a", function() {
        $(this).siblings().removeClass('hidden');
        return $(this).addClass('hidden');
      });
      return $('.toggling-panel').on("click", "a[data-method='delete']", function() {
        var res;
    ...
    return root.gift_function = function(iUrl) {
      var res;
...
(function() {
  ...
  jQuery(function() {
    return $(document).on("page:change", function() {
      if ($('#layout-user-index').length) {
        return root.UserIndex();
      } else if ($('#layout-user-new').length || $('#layout-user-edit').length) {
        return root.UserNew();
      } else if ($('#layout-user-show').length) {
        return root.UserShow();
      }
    });
  });
